...
# streamlit页面
tab1, tab2, tab3 = st.tabs([":books:故障报告", ":open_book:官方手册", ":notebook_with_decorative_cover:运维文档"])
with tab1:
  st.subheader("上传Oracle故障报告")
  uploaded_files = st.file_uploader("请选择上传的文件", type=["pdf","txt","docx"], accept_multiple_files=True, key='issue_uploader')
  issue_chunk_size = st.number_input("chunk_size参数:", min_value=100, max_value=2048, value=1000, key='issue_chunk_size')
  issue_chunk_overlap = st.number_input("chunk_overlap参数:", min_value=0, max_value=400, value=100, key='issue_chunk_overlap')
  	  ...
  	  with st.spinner("正在加载文件、切片和嵌入向量中，请等待"):
  	    for uploaded_file in uploaded_files:
  	  	  ...
  	  	  if extension == ".pdf":
  	  	    ...
  	  	    chunks = chunk_data(data, chunk_size=issue_chunk_size, chunk_overlap=issue_chunk_overlap)
...
with tab2:
  ...
  if uploaded_books and book_upload_button:
  	  with st.spinner("正在加载文件、分块和嵌入向量中，请等待"):
